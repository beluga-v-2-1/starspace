...
curs=c.cursor()
m=0
#here we have imported mysql this program requires it
#the database name is not set yet and
#the password is "password" change it if need be.. 
def end():
...
#        time.sleep(0.001)
#    print()
#these "ty" commands were initially used to slow down the
#printing/displaying of letters to give a more old school feel
#but ive now replaced it with just regular print to make it
#easier to learn
def addpeople():
    nott=0
    print("Alright... lets add new employees")
    print()
    ...
    n3=input(">>>")                                   #enter a phone number
    if n1.lower()=="end":
        print("command detected, moving to MAIN MENU")
        startscreen()
    elif n3.isdigit()==False:
            print("try using your brain and enter a proper phone ?")
            addpeople()
    print("enter sex")                                #enter the gender
    n4=input(">>>")
    if n1.lower()=="end":
        print("command detected, moving to MAIN MENU")
        startscreen()
        ...
        nott+=1                                     #trying to find incorrect values
    elif n4=="F":
        nott+=1
    elif n4=="f":
        nott+=1
        ...
        print("try  entering either m or f ")
        addpeople()                                 
    print("enter a age ")
    n5=input(">>>")                                 #enter the age
    if n1.lower()=="end":
        print("command detected, moving to MAIN MENU")
        startscreen()
    elif n5.isdigit()==False:                       #enterring 
        print("try using your brain and enter a proper number ?")
        addpeople()
                                                    #enterring admissionnumber,name,phonennumber,gender,age into the table "employeetest"
    curs.execute("insert into employeetest values('{}','{}','{}','{}','{}')".format(n1,n2,n3,n4,n5))
    c.commit()
                                                    #since we are using python-mysql connector, the autocommit is turned off by default
                                                    #so we have to manually turn it on by using "c.commit"
    curs.execute("select * from employeetest")      #to list out the table again
...
def specview():
    try:
        ...
        if n.lower()=="end":
            ...
            print("enter any charecter")                              #try exception block is used here to catch any errors and prevent the
